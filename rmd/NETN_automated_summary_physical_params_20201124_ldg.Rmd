...
comp_plots_final <- subplot(comp_plots[1:length(comp_plots)],
  ...
  style(showlegend = FALSE) #%>% layout(autosize = FALSE, width = 910, height = 650)
...
```{r, site_comp_plots, results = 'show', out.width = "100%"}
...
```{r site_tabs, results = 'asis', warning = F, message = F, out.width = "100%"}
for(i in seq_along(site_list)){
 ...
 # fig_cap_list <- c(rep(NA, length(plot_list[names(plot_list) == site])-1, paste0(fig_cap)))
  ...
  #                  width = 900, 
  #                  height = 
  #                    ifelse(length(plot_list[names(plot_list) == site])> 4, 650, 450))
  #                   )
  #         ))
