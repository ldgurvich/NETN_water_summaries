...
char <- "DOC_mgL"
  ...
  all_sites_plotly <- ggplotly(lineplot, tooltip = c("text")) #%>% 
    #add_trace(y = ~sitename, name = sitename) #plotly legend
...
site_names <- getSiteInfo(netnwd, parkcode = params$park, info = "SiteName")
char_list <- getCharInfo(netnwd, park = params$park, sitecode = site_list, category = params$cat, info = "CharName") %>% 
             .[duplicated(.)]  %>% unique()
...
```{r site_1, echo=F, message=FALSE, warning=FALSE, fig.height=6, out.width='100%'}
