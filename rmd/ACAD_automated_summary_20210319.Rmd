...
# Figure caption for comp plot tabs
fig_cap_comp <- htmltools::withTags(body(
...
# Figure caption
if(params$nutrient==TRUE) {cat(as.character(fig_cap_comp))}
```
...
char_df_lake_p <- map(site_list, ~getCharInfo(netnwd, parkcode = params$park, sitecode = ., 
                             category = params$cat, info = "CharName")) %>%  
                 set_names(site_list) %>% unlist() %>% as.data.frame() 
                 #unlisting matches a site to each param

# Reorganize dataframe                 
char_df_lake_p$site_code <- substr(row.names(char_df_lake_p), 1, 14) #select the site code
colnames(char_df_lake_p) <- c("char_name", "site_code") #rename columns
rownames(char_df_lake_p) <- c() #remove row names
char_df_lake_p <- char_df_lake_p[c("site_code", "char_name")] #rearrange column order
