...
#include <- ifelse(params$cat == 'physical', T, F)

```

```{r glob_vars}

# often used special symbols
mS <- htmltools::HTML("&#181;&#83;", .noWS = "outside") #microsiemens
degree <- htmltools::HTML("&#176;", .noWS = "outside")

```

```{r temp_lib, warning = F}
library(htmltools)
library(kableExtra)
```
...
methods <- htmltools::withTags(body(
      p(h3("Water Sample Collection:"), "Stream water is collected by submerging the sample bottle directly in the centroid of the streamflow with a gloved hand. Samples are analyzed for Acid Neutralizing Capacity (ANC), total phosphorus (TP), total nitrogen (TN), and dissolved organic carbon (DOC)."),
...
physical <- htmltools::withTags(body(
  ...
  # Water temp
  p(h3("Water Temperature:"), "Several of the water chemistry parameters are water temperature dependent, such as DO and specific conductance. High temperature can also stress aquatic organisms, particularly those adapted to habitats with cooler temperatures such as trout. Temperature is reported in ", degree, "C."),
...
nutrients <- htmltools::withTags(body(
  ...
  # ANC
  p(h3("Acid Neutralizing Capacity (ANC):"), "ANC is a measure of the amount of compounds in the water that neutralize low pH, making it the prime indicator of a waterbody’s susceptibility to acid inputs. The higher the ANC of a waterbody, the better equipped it is to resist the effects of acid. The measured ANC refers to the alkalinity of an unfiltered water sample. This is different than the pH of the stream, as pH measures acidity, but does not measure buffering capacity. ANC is typically caused by anions (negatively charged particles) in natural waters that can enter into a chemical reaction with a strong acid. These are primarily the carbonate", HTML("(CO<sub>3</sub><sup>2-</sup>)"), "and bicarbonate", HTML("(HCO<sub>3-</sub>)"), "ions. Borates, phosphates, silicates, arsenate, and ammonium can also contribute to alkalinity when present."),
...
ct <- htmltools::withTags(body(
  p(h3("Connecticut State Water Quality Standards:"), "Connecticut water quality standards define ranges of total phosphorus, total nitrogen, chlorophyll a, and Secchi disk transparency which are assessed collectively to determine the trophic state of a lake or pond. For the purpose of determining consistency with the water quality standard, the natural trophic state of a lake or pond is compared with the current trophic state to determine if it has been altered due to excessive anthropogenic inputs. Lakes and ponds in advanced trophic states which exceed their natural trophic state due to anthropogenic sources are considered to be inconsistent with water quality standards.")
  
    ))

cat(as.character(ct))

htmltools::withTags(body("Connecticut lake and pond water quality standards"))
...
me <- htmltools::withTags(body(
  p(h3("Maine State Water Quality Standards:"), "All waters in national parks are considered outstanding national resource waters (ONRW) in Maine. The water quality of ONRW must be maintained and protected. Class AA is the highest classification and its standards shall be applied to waters that are outstanding natural resources."),
...
htmltools::withTags(body("Maine stream water quality standards"))
...
htmltools::withTags(body("Maine lake and pond water quality standards"))
...
nh <- htmltools::withTags(body(
  p(h3("New Hampshire State Water Quality Standards:"), 'Surface waters designated as natural are considered outstanding resource waters (ORW) in New Hampshire. “Water quality shall be maintained and protected in surface waters that constitute ORW”. Class A is the most protective surface water classification in New Hampshire’s Surface Water Quality Regulations.'),
...
htmltools::withTags(body("New Hampshire stream water quality standards"))
...
htmltools::withTags(body("New Hampshire lake and pond water quality standards"))
...
htmltools::withTags(body("New Jersey stream water quality standards"))
...
htmltools::withTags(body("New York stream water quality standards"))
...
htmltools::withTags(body("Vermont stream water quality standards"))
...
htmltools::withTags(body("Vermont lake and pond water quality standards"))
