...
fig <- ggplot(wdat, aes(x=Date, y=ValueCen)) +
    #geom_boxplot(color="#1378b5", fill="#76b4db", alpha = 0.85,
                #outlier.color = "orange") +
...
```{r boxplot_functions, warning=F, message=F}
...
boxplot_list <- map2(site_char_list$sitecode, site_char_list$charname, 
...
subplots <- function(plot_list, num_plots){
    subplot(plot_list[names(plot_list) == site][1:num_plots],
...
for(i in seq_along(site_list)){
  ...
  num_plots <- length(boxplot_list[names(boxplot_list) == site])
  
  plot_title <- htmltools::withTags(h2('Insert title'))
                                                 
  fig_cap <- htmltools::withTags(body('Caption goes here.'))
  
  fig_height <- ifelse(num_plots > 4, 6.5, 4.5)
  fig_width <- 9.3
  
  pt_subplots <- subplots(ptplot_list, num_plots)
  bp_subplots <- subplots(boxplot_list, num_plots)
  
  # Logical way to find and turn on poor water quality and wq thresholds
  # First turn them all off
  for (i in seq(1:length(pt_subplots$x$data))) {
    pt_subplots$x$data[[i]]$showlegend <- FALSE
  }
  
  start <- (length(pt_subplots$x$data))-(length(params$from:params$current))+1
  
  for (i in start:length(pt_subplots$x$data)){
    pt_subplots$x$data[[i]]$showlegend <- TRUE
  }

  # THE PROBLEM IS....the colors don't match in every plot
