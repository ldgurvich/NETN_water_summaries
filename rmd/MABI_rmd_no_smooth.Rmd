...
water_plot <- function(site, char){
  ...
  monthly_plot <- ggplot(data = final_data, aes(x = mon_num, y = median_val))+
    ...
    geom_line(aes(x = mon_num, y = lower_100, 
                  text = paste0("Historic ", month, " Mininum: ", "<br>", param_name, ": ", round(lower_100, 1), " ", unit)),
              color = "#89A7E7", alpha = 0.3, lwd=3)+
...
```
### Site Comparisons
<h2><b>Comparisons across sites (`r params$current`)</b></h2>
```{r, echo = F, warning = F, message = F, fig.height=6, out.width='100%'}
...
plots <- purrr::map(char_list, ~all_sites_plot(park, site_list, char = .)) %>%
...
```
<img src="./images/legend_MABI_sites_wide.png" alt="plot legend" width="40%">
...
### Pogue Brook
<h2><b>Historic (`r params$from`&ndash;`r params$to`) vs. current (`r params$current`) measurements</b></h2>
```{r, echo = F, warning = F, message = F, fig.height = 9, out.width = '100%'}
park <- params$park
site <- "NETN_MABI_SA00"
sitename <- getSiteInfo(netnwd, parkcode = park, sitecode = site, info = "SiteName")
...
plots_final
```
<img src="./images/legend_wide.png" alt="plot legend" class="legend_size">
