...
```{r, echo = F, warning = F, message = F, out.width='98%', fig.asp=1.2, fig.align='center'}
park <- params$park
site <- "NETN_MABI_SA00"
sitename <- getSiteInfo(netnwd, parkcode = park, sitecode = site, info = "SiteName")
...
plots <- purrr::map(char_list, ~water_plot(site = "NETN_MABI_SA00", char = .)) %>%
...
#                      draw_image(leg_fig, x = 0.6, y = 0.1, height = 0.15, width = 0.35)
# 
# plot_final

plots_final <- subplot(plots$DO_mgL, plots$Temp_C, plots$SpCond_uScm, plots$Discharge_cfs, plots$pH,
                       titleY = TRUE, titleX = FALSE, margin = 0.05, nrows=3)

plots_final
```
<img src="./images/legend_wide.png" alt="plot legend" width="85%">
...
plots <- purrr::map(char_list, ~water_plot(site = "NETN_MABI_PA00", char = .)) %>%
        set_names(c(char_list)) #name plots to refer to them by char name
