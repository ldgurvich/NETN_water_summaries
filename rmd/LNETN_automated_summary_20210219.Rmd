...
p_char_list <- getCharInfo(netnwd, parkcode = params$park, category = params$cat, 
                         info = "CharName") %>% #.[duplicated(.)] %>% 
                         unique()

p_legend_list <- c(rep("none", length(p_char_list)-1), "bottom")

p_comp_plots <- purrr::map2(p_char_list, p_legend_list,
                         ~watersite_comps(netnwd, year = params$current, 
                                     parkcode = params$park, 
                                     sitecode = site_list,
                                     charname = .x,
                                     legend = .y)) %>%
              set_names(p_char_list) #name plots to refer to them by char name

p_comp_plots_final <- subplot(p_comp_plots[1:length(p_comp_plots)],
                         titleY = TRUE, titleX = FALSE, margin = 0.05,
                         nrows = ifelse(length(p_comp_plots)> 4, 3, 2),
...
for (i in 1:length(num_sites)){
  p_comp_plots_final$x$data[[i]]$showlegend <- TRUE #first set of site elements 
  p_comp_plots_final$x$data[[length(p_comp_plots_final$x$data)]]$showlegend <- TRUE #last element
  p_comp_plots_final$x$data[[length(p_comp_plots_final$x$data)]]$name <- "WQ Threshold" 
}

p_comp_plots_final <- p_comp_plots_final %>% layout(legend = list(x = 0, y = -0.05, orientation = "h"))

##------------------------------------------------------------------------------
##                        Nutrients site comp setup
##------------------------------------------------------------------------------

n_char_list <- c("TN_mgL", "TP_ugL")

n_legend_list <- c(rep("none", length(n_char_list)-1), "bottom")

n_comp_plots <- purrr::map2(n_char_list, n_legend_list,
                         ~watersite_comps(netnwd, 
                                          ...
                                          charname = .x,
                                          legend = .y)) %>%
              set_names(n_char_list) #name plots to refer to them by char name

n_comp_plots_final <- subplot(n_comp_plots[1:length(n_comp_plots)],
...
for (i in 1:length(num_sites)){
  n_comp_plots_final$x$data[[i]]$showlegend <- TRUE #first set of site elements 
  n_comp_plots_final$x$data[[length(n_comp_plots_final$x$data)]]$showlegend <- TRUE #last element
  n_comp_plots_final$x$data[[length(n_comp_plots_final$x$data)]]$name <- "WQ Threshold" 
}

n_comp_plots_final <- n_comp_plots_final %>% layout(legend = list(x = 0, y = -0.12, orientation = "h"))
...
if(params$nutrient==TRUE) {n_comp_plots_final}
```
...
```{r templates, warning=F, message=F}
