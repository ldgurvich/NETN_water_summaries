...
#   group_by(Site) %>% 
#   mutate(start_date = format(min(Date), "%b %Y")) %>% 
#   mutate(last_date = format(max(Date), "%b %Y")) %>%
#   ungroup() %>% 
#   group_by(Site, start_date, last_date) %>% 
#   summarize()

years_active <- wdata %>% group_by(Site) %>%
...
water_sites <- data.frame(ParkCode = getSiteInfo(netnwd, parkcode = params$park, info = "ParkCode"),       
                          ...
                          type = as.factor(getSiteInfo(netnwd, parkcode = params$park, info = "type"))
                          )

water_sites <- water_sites %>% left_join(years_active, by = c("SiteCode" = "Site"))
  
# water_icon <- makeAwesomeIcon(icon = "tint",
#                               markerColor = ifelse(water_sites$type == "Stream", "blue", "gray"),
#                               iconColor = "white",
#                               library = "fa")

water_icon_set <- awesomeIconList(
  "Stream" = makeAwesomeIcon(icon = "tint", markerColor = ifelse(water_sites$type == "Stream", "blue", "gray"),
                             iconColor = "white", library = "fa"),
  "Lake" = makeAwesomeIcon(icon = "tint", markerColor = ifelse(water_sites$type == "Stream", "blue", "gray"),
                             iconColor = "white", library = "fa")
)
...
#                                "<ion-icon style='margin-left: 8px; margin-top: 11px;'
#                                name='waterdrop'></ion-icon>",
...
leaflet() %>%
    ...
    addAwesomeMarkers(data = water_sites, ~long, ~lat, icon= ~water_icon_set[type], 
                      label=as.character(water_sites$SiteName),
                      popup = paste0(water_sites$SiteName, "<br>", 
                                     "Type: ", water_sites$type, "<br>",
                                     "Active: ", water_sites$start_date, "&ndash;", water_sites$last_date)) %>% 
    addControl(markerLegendHTML(IconSet = water_icon_set), position = "bottomleft")
