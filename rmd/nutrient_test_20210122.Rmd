...
boxplot <- function(sitecode, charname) {
  ...
  # Remove months that were not regularly sampled before 2018
   if (wdat$SampleDepth == "epilimnion") {
    wdat <- filter(wdat, month == "Jun" | month == "Aug")} else {
       wdat <- filter(wdat, month == "May" | month == "Aug")
     }
  
  # Subset wdat into historic and current 
  ...
  param_name <- getCharInfo(netnwd, parkcode = params$park, sitecode = sitecode,
                            charname = charname, info = "CategoryDisplay") %>% unique()

  unit <- getCharInfo(netnwd, parkcode = params$park, sitecode = sitecode, 
                      charname = charname, info = "Units") %>% unique()

  yname <- ifelse(charname != "pH", 
                  paste0(getCharInfo(netnwd, parkcode = params$park, sitecode = sitecode,
                                     charname = charname, info = "CategoryDisplay") %>% unique(), " (", 
                         getCharInfo(netnwd, parkcode = params$park, sitecode = sitecode,
                                     charname = charname, info = "Units") %>% unique(),                           
                         ")"),
                  paste0(getCharInfo(netnwd, parkcode = params$park, sitecode = sitecode,
                                     charname = charname, info = "CategoryDisplay") %>% unique()))
  ...
  p <- ggplot(wdat, aes(x=month, y=ValueCen_hist)) +
      geom_boxplot(color="#1378b5", fill="#76b4db", alpha = 0.85) +
      geom_point(aes(y=ValueCen_curr)) +#, color="black") +
      forestMIDN::theme_FVM() +
      labs(y = yname, x = NULL)
...
#site_char_list <- site_char_list[-c(5, 9), ] #temporarily remove DOC so function doesn't crash
